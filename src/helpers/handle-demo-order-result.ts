...
// const rateInsurWin = parseFloat(process.env.RATE_INSURANCE_WIN) || 0.25
...
export async function processOrderDemo(
  ...
  const createOrderResultData = await Promise.all(
    currentRoundOrders.map(async (order) => {
      ...
      // if (await isUseInsurance(prisma, order.user_id, order.account_type)) {
      //   winAmount = math
      //     .sub(winAmount, math.mul(order.bet_amount, rateInsurWin).toNumber())
      //     .toNumber()
      // }
...
// async function isUseInsurance(
//   prisma: PrismaClient,
//   userId: string,
//   accountType: AccountType,
// ) {
//   return await prisma.userInsurance.findFirst({
//     where: {
//       user_id: userId,
//       is_enable: true,
//     },
